...
class Account_details_model extends CI_Model {
	...
	public function get_by_filter($country, $city, $category, $place, $limit = null, $offset = null)
	{
		$limit = $this->config->item('limit');

		$this->db->select('*');
		$this->db->from('a3m_account_details');
		//$this->db->where('profile_type !=', 'city');

		if($country)
		{
			$this->db->where('current_country', $country);

			if($city)
			{
				$this->db->where('current_city', $city);

				if($category)
				{
					$this->db->where('profile_type', $category);

					if($place)
					{
						$this->db->where('company_name', $place);
					}
				}
			}
		}
