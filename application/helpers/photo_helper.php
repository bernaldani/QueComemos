...
function showPhoto($picture = NULL, $param = NULL)
{
	...
	if (isset($picture) && strlen(trim($picture)) > 0)
	{
		...
		if ( ! $remote)
		{
			...
			$path = site_url(IMG_DIR.'/user/profile/'.$picture); //.		-- disabled time attachment, no need to break cache
		}
		else
		{
		...
		if ($check && ! $remote)
		{
			if ( ! fileExists($path))
			{
				...
				$path = site_url(RES_DIR.'/images/default-person.png');
			}
		}
	...
	return '<img '.(($id) ? 'id="'.$id.'"' : '').' src="'.$path.'" height="'.$height.'" width="'.$width.'" title="'.url_title($title,'_').'" alt="'.url_title($title,'_').'" class="'.$class.'">';

}

function showImage($filename,$param = NULL)
{
	$height = (isset($param['height'])) ? $param['height'] : 100;
	$width = (isset($param['width'])) ? $param['width'] : 100;
	$id = (isset($param['id'])) ? $param['id'] : FALSE;
	$nocache = (isset($param['nocache'])) ? $param['nocache'] : FALSE; // TRUE = disable caching, add time string to image url
	$title = (isset($param['title'])) ? $param['title'] : "Image";
	$class = (isset($param['class'])) ? $param['class'] : "thumbnail";
	$thumbs = (isset($param['thumbs'])) ? $param['thumbs'] : "";

	$path = site_url(IMG_DIR.'/'.$thumbs.'/'.$filename);
