...
class Template
{
	...
	/**
	 * Put extra javascipt in the footer
	 *
	 * @access	public
	 * @param	 string	$line The line being added to footer
	 * @return	void
	 */
	public function append_fmetadata($line)
	{
		$this->_fmetadata[] = $line;
		return $this;
	}
	 ...
	 * @return	void
	 */
	public function set_fmetadata($name, $content, $type = 'meta')
	{
		$name = htmlspecialchars(strip_tags($name));
		$content = htmlspecialchars(strip_tags($content));
		...
		{
			case 'meta':
				$this->_fmetadata[$name] = '<meta name="'.$name.'" content="'.$content.'" />';
			break;

			case 'link':
				$this->_fmetadata[$content] = '<link rel="'.$name.'" href="'.$content.'" />';
			break;
		}
