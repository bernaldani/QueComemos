...
class Connect_twitter extends My_Controller {
	...
	function __construct()
	{
		...
		$this->page_title = 'Twitter Connect';
	...
	function index()
	{
		...
		if ($this->input->get('oauth_token'))
		{
			...
			// Check if user has connect twitter to Worthy.at
			if ($user = $this->account_twitter_model->get_by_twitter_id($twitter_info['id']))
			{
				// Check if user is not signed in on Worthy.at
			...
			// The user has not connect twitter to Worthy.at
			else
			{
				// Check if user is signed in on Worthy.at
				if ( ! $this->authentication->is_signed_in())
				{
					...
					$this->session->set_userdata('connect_create', array(array('provider' => 'twitter', 'provider_id' => $twitter_info['id'], 'username' => $twitter_info['screen_name'], 'token' => $twitter_token->oauth_token, 'secret' => $twitter_token->oauth_token_secret), array('picture' => $twitter_info['profile_image_url'])));

					// Create Worthy.at account
					redirect('account/connect_create');
				}
				else
				{
					// Connect twitter to Worthy.at
