...
class Connect_facebook extends My_Controller {
	...
	function __construct()
	{
		...
		$this->page_title = 'Facebook connect';
	...
	function index()
	{
		...
		{
			// Check if user has connect facebook to Que comemos
			if ($user = $this->account_facebook_model->get_by_facebook_id($this->facebook_lib->user['id']))
			{
				// Check if user is not signed in on Que comemos
			...
			// The user has not connect facebook to Que comemos
			else
			{
				// Check if user is signed in on Que comemos
				if ( ! $this->authentication->is_signed_in())
				{
					// Store user's facebook data in session
					$this->session->set_userdata('connect_create', array(array('provider' => 'facebook', 'provider_id' => $this->facebook_lib->user['id'], 'email' => $this->facebook_lib->user['email']), array('fullname' => $this->facebook_lib->user['name'], 'firstname' => $this->facebook_lib->user['first_name'], 'lastname' => $this->facebook_lib->user['last_name'], //'gender' => $this->facebook_lib->user['gender'], //'dateofbirth' => $this->facebook_lib->user['birthday'],	// not a required field, not all users have it set
					...
					// Create Que comemos account
					redirect('account/connect_create');
				}
				else
				{
					// Connect facebook to Que comemos
